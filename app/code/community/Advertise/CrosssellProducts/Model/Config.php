 ...
 */
class Advertise_CrosssellProducts_Model_Config extends Varien_Object
{
    /**
     * Config keys
     */
    //const ENABLED  =         'importer/importer_group/enabled';
    const PRODUCT_COUNT =    'advertise_suggestedproducts_options/advertise_suggested_products/advertise_crosssell_prod_count';
    ...
    public function getDownloadUrl()
    {
        ...
        Mage::log("Crosssell Products getDownloadURL self::DOWNLOAD_URL=".Mage::getStoreConfig(self::DOWNLOAD_URL));
        // FOR PRODUCTION
        $advertiseEmail = $this->getAdvertiseEmail();
        if( ! empty($advertiseEmail)) {
            Mage::log("Crosssell Products getDownloadURL returning (1): ".$this->getAdvertiseFeed());
            return $this->getAdvertiseFeed();
        }
        Mage::log("Crosssell Products getDownloadURL returning (2): ".Mage::getStoreConfig(self::DOWNLOAD_URL));
